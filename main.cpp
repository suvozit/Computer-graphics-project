...
//extra
#include <mmsystem.h>   // add this at the top of your file
...
// lip
    ...
    glVertex2f(798-15, 215);

    glVertex2f(810-18, 215);
...
void seen_three()
{
    ...
    for (int x=50; x<=1150; x+=100)
    {
        glVertex2f((GLfloat)x,450);
        glVertex2f((GLfloat)x,470);
    }
...
void initGL()
{
    ...
    glLoadIdentity();
    gluOrtho2D(0, 1200, 0, 750);           // same for both
}

void updateYort(int value)
{
    yort1X += yort1Speed;
    if (yort1X < -750)
    {
...
{
    if (isseen_two) {       // only move birds in scene 2
        birdX += birdSpeed; // update position
    }

    glutPostRedisplay();            // redraw
    glutTimerFunc(16, updateBird, 0); // call again in ~16 ms
}
...
/*void handleKeys(unsigned char key, int x, int y)
{
    ...
    // ✅ Bird controls should be always checked, not hidden inside 't'
    if(isseen_two)
    {
        switch (key)
        {
        case 'd':
            birdSpeed = 3.0f;   // move bird right
            break;
        case 'a':
            birdSpeed = -3.0f;  // move bird left
            break;
        case ' ':
            birdSpeed = 0.0f;   // stop bird
            break;
        }
    }
...
void handleKeys(unsigned char key, int x, int y)
{
    ...
    if(key=='t')   // Scene 2
    {
        ...
        PlaySound(TEXT("E:\\Mergdemo\\bin\\Debug\\scene2.wav"), NULL, SND_ASYNC | SND_LOOP | SND_FILENAME);
    }
    ...
    if(key=='h')   // Scene 3
    {
        ...
        isseen_three=true;
        PlaySound(NULL, 0, 0);  // stop previous sound
        PlaySound(TEXT("E:\\Mergdemo\\bin\\Debug\\scene3.wav"), NULL, SND_ASYNC | SND_LOOP | SND_FILENAME);
    }

    if(key=='1')   // Go back to Scene 1 manually
    {
        isseen_one=true;
        isseen_two=false;
        isseen_three=false;
        PlaySound(NULL, 0, 0);  // stop previous sound
        PlaySound(TEXT("E:\\Mergdemo\\bin\\Debug\\scene1.wav"), NULL, SND_ASYNC | SND_LOOP | SND_FILENAME);
    }

    // ✅ Bird controls only in scene 2
    if(isseen_two)
    {
        switch (key)
        {
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("Merged Motion + Keyboard Control");
    initGL();
    glutDisplayFunc(display);      // display
    glutKeyboardFunc(handleKeys);  // merged keyboard

    glutTimerFunc(0, updateYort, 0);
    glutTimerFunc(0, updateBoat, 0);
    glutTimerFunc(0, updateCloud, 0);
    glutTimerFunc(0, updateBird, 0);

    PlaySound(TEXT("E:\\Mergdemo\\bin\\Debug\\scene1.wav"), NULL, SND_ASYNC | SND_LOOP);
